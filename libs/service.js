...
exports.getToday = function (cb) {
    ...
    //根据时间范围进行查找
    Order.getOrdersByQuery({time: {$gt: start_t, $lt: end_t}}, {}, function (err, orders) {
        if (!err) {
            if (orders.length == 0) {
                cb("null");
                return;
                ...
                if (!order.canceled) {
                    ...
                    for (var j = 0; j < order.order.length; j++) {
                        ...
                        if (group[shop_id].analytics[order.order[j].id] == undefined) {
                            group[shop_id].analytics[order.order[j].id] = {
                                name: order.order[j].name,
                                num: parseFloat(orders[i].order[j].num)
                            };
                        } else {
            ...
            var forEach = function (i) {
                ...
                if (!group[shop_id]) {
                    Shop.getShopById(shop_id, function (err, shop) {
                        group[shop_id] = {
                            shop: shop,
                            totalPrice: 0,
                            totalNum: 0,
